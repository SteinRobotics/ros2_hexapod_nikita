...
class BusServoProtocol {
   public:
    BusServoProtocol(std::shared_ptr<rclcpp::Node> node, std::string deviceName);
    virtual ~BusServoProtocol() = default;
    ...
    // Generic Commands
    bool getServoID(uint8_t ID, uint8_t& answer);
    bool setServoID(uint8_t ID, uint8_t newID);        // set new ID for servo, 0..254
    bool getMode(uint8_t ID, uint8_t& mode);           // get servo mode, 0 = servo, 1 = motor
    bool setServoMode(uint8_t ID);                     // set servo mode
    bool setMotorMode(uint8_t ID, int16_t speed = 0);  // set motor mode, speed in 0.1°/s, -1000..1000
    bool setTorque(uint8_t ID, bool active);           // set motor torque active

    // Position
    bool setPosition(uint8_t ID, uint16_t pos, uint16_t timeMs);  // move to pos 0..1000 (=0..240°)
    bool setRegPos(uint8_t ID, uint16_t pos, uint16_t timeMs);    // same as above but wait until actionStart
    bool actionStart(
        uint8_t ID =
    ...
    bool getPosition(uint8_t ID, int16_t& pos);  // get current pos 0..1000 (= 0~240°)
    bool getPositionLimits(uint8_t ID, uint16_t& min_position, uint16_t& max_position);
    bool setPositionLimits(uint8_t ID, uint16_t min_position, uint16_t max_position);
    bool getPositionOffset(uint8_t ID, int8_t& deviation);  // get position offset in °, 0..1000 = 0°..240°
    bool setPositionOffset(uint8_t ID, int8_t deviation);   // set position offset in °, 0..1000 = 0°..240°
    bool savePositionOffset(uint8_t ID);  // save position offset in servo flash, use after setPositionOffset

    // Motor Control
    bool getMotorSpeed(uint8_t ID, int16_t& speed);  // get current motor speed in 0.1°/s, -1000..1000

    // Voltage
    bool getVoltage(uint8_t ID, uint16_t& mV);  // get current Vin 4500mV..12000mV
    bool getVoltageLimits(uint8_t ID, uint16_t& mVmin, uint16_t& mVmax);
    bool setVoltageLimits(uint8_t ID, uint16_t mVmin, uint16_t mVmax);

    // Temperature
    bool getTemperature(uint8_t ID, uint8_t& degTemp);  // get current Temp 0..100°C
    bool getMaxTemperatureLimit(uint8_t ID, uint8_t& degLimit);
    bool setMaxTemperatureLimit(uint8_t ID, uint8_t degLimit);  // set max temperature limit in °C

    // LED
    bool isLedOn(uint8_t ID);          // check if LED is on
    bool setLed(uint8_t ID, bool on);  // set LED on or off

    // Error handling
    // get LED error code, b0=temp, b1:voltage, b2:stalled -> also clears errors?
    bool getLedErrcode(uint8_t ID, uint8_t& lederrcode);
    bool flashLedErrCode(uint8_t ID, uint8_t code);

   private:
    bool initializeSerialConnection(const std::string& deviceName_);
    std::shared_ptr<rclcpp::Node> node_;  // only for logging
