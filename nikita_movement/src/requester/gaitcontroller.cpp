...
CGaitController::CGaitController(std::shared_ptr<rclcpp::Node> node, std::shared_ptr<CKinematics> kinematics)
    ...
    gait_tripod_ = std::make_unique<nikita_movement::CTripodGait>(
        node_, kinematics_, LEG_LIFT_HEIGHT, GAIT_STEP_LENGTH, FACTOR_VELOCITY_TO_GAIT_CYCLE_TIME);
    gait_ripple_ = std::make_unique<nikita_movement::CRippleGait>(node_, kinematics_);
    gait_bodyroll_ = std::make_unique<nikita_movement::CBodyRollGait>(node_, kinematics_);
    gait_legroll_ = std::make_unique<nikita_movement::CLegRollGait>(node_, kinematics_);

    // Default active gait
    active_type_ = EGaitType::Tripod;
...
void CGaitController::setGait(EGaitType type) {
    if (type == active_type_ && active_gait_ != nullptr) {
        return;
    }
    active_type_ = type;
    switch (type) {
        ...
        case EGaitType::BodyRoll:
            ...
            break;
        case EGaitType::LegRoll:
            active_gait_ = gait_legroll_.get();
            break;
    }
    active_gait_->start();
}

void CGaitController::updateSelectedGait(const geometry_msgs::msg::Twist& velocity, CPose body) {
    if (active_gait_) {
        active_gait_->update(velocity, body);
    }
}
...
void CGaitController::cancelStopSelectedGait() {
    if (active_gait_) {
