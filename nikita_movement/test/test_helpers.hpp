...
inline void expectLegNear(const CLeg& expected, const CLeg& actual, double tol, const std::string& msg = "") {
...
inline void expectHeadNear(const CHead& expected, const CHead& actual, double tol,
                           const std::string& msg = "") {
    EXPECT_NEAR(expected.degYaw, actual.degYaw, tol) << msg;
    EXPECT_NEAR(expected.degPitch, actual.degPitch, tol) << msg;
}
